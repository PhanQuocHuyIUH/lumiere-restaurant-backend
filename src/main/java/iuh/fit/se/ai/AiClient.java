...
public class AiClient {
    ...
    private static final long BACKOFF_JITTER_MS = 100L;
        ...
        for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            try {
                ...
                if (attempt < MAX_ATTEMPTS) {
                    ...
                    LOGGER.warn("AI call attempt {}/{} failed. Retrying in {}ms: operation={}, path={}, reason={}",
                            attempt,
                            MAX_ATTEMPTS,
                            delayMs,
                            operation,
                            path,
                            safeMessage(ex));
                    if (!sleepUninterruptibly(delayMs)) {
    ...
    private long computeBackoffMs(int attempt) {
        long exp = BACKOFF_BASE_MS * (1L << (attempt - 1));
        long capped = Math.min(exp, BACKOFF_MAX_MS);
        long jitter = ThreadLocalRandom.current().nextLong(BACKOFF_JITTER_MS + 1);
